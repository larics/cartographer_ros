...
Detector::Detector() : last_optimization_epoch_(-1) {
  frontier_publisher_ = ros::NodeHandle().advertise<visualization_msgs::Marker>("frontier_marker", 1, true);
}

void Detector::handleNewSubmapList(const cartographer_ros_msgs::SubmapList::ConstPtr &submap_list) {
  //last_submap_list_ = submap_list;
  std::unique_lock<std::mutex> lock(mutex_);
  submap_poses_.clear();

  for(const auto& entry : submap_list->submap) {
    submap_poses_.emplace(std::piecewise_construct,
...
inline Eigen::Array2i GetCellIndex(
  ...
  // rotate.
  const auto point_local = (texture.slice_pose.inverse() * submap_pose.inverse() * global).translation();
  return Eigen::Array2i(
      cartographer::common::RoundToInt( (-point_local.x() / texture.resolution - 0.5)),
      cartographer::common::RoundToInt( (-point_local.y() / texture.resolution - 0.5)));
}
inline cartographer::transform::Rigid3d GetGlobal(
  ...
  // and contains (centered_max_x, centered_max_y). We need to flip and
  // rotate.
  return submap_pose * texture.slice_pose * cartographer::transform::Rigid3d::Translation({
      -(cell_index[0] + 0.5) * texture.resolution,
      -(cell_index[1] + 0.5) * texture.resolution,
      0});
}

inline Eigen::Array2i FromFlatIndex(const int flat_index, const cartographer::io::SubmapTexture& texture) {
  return { flat_index / texture.width, flat_index % texture.width };
}

inline int ToFlatIndex(const Eigen::Array2i& cell_index, const cartographer::io::SubmapTexture& texture) {
  return cell_index[0] * texture.width + cell_index[1];
}

bool Contains(const Eigen::Array2i& cell_index, const cartographer::io::SubmapTexture& texture) {
  return (Eigen::Array2i(0, 0) <= cell_index).all() &&
      (cell_index <
          Eigen::Array2i(texture.height, texture.width))
          .all();
}
...
void Detector::publishUpdatedFrontiers() {
  ...
  for (const auto& submap_i : submap_poses_) {
    ...
    color.g = 1.0;
    cartographer::transform::Rigid3d pose = submap_i_pose * submap_i_texture.slice_pose *
        cartographer::transform::Rigid3d::Translation({
                                                            -0.5 * submap_i_texture.resolution,
                                                            -(submap_i_texture.width - 1 + 0.5) * submap_i_texture.resolution,
                                                            0});
    frontier_point.x = pose.translation().x();
    frontier_point.y = pose.translation().y();
    ...
    for (auto& frontier_cell : frontier_cells) {
      const auto global = GetGlobal(submap_i_pose, frontier_cell.first, submap_i_texture);
      auto& submap_hint = frontier_cell.second;

      bool ok = true;

      const auto validate_submap = [&](const cartographer::mapping::SubmapId& id_j) {
        ...
        const auto cell_index = GetCellIndex(submap_j_pose, global, submap_j_texture);
        const int dx[]{-1, -1, -1, 0, 0, 0, 1, 1, 1};
        const int dy[]{-1, 0, -1, -1, 0, 1, -1, 0, 1};
        for (int i = 4; i < 5; i++) {
          const Eigen::Array2i cell_index_d{cell_index[0] + dx[i], cell_index[1] + dy[i]};
          const auto flat_index = ToFlatIndex(cell_index_d, submap_j_texture);
          if (Contains(cell_index_d, submap_j_texture)
              && (static_cast<unsigned char>(submap_j_texture.pixels.intensity.at(flat_index)) == 255 ||
                  static_cast<unsigned char>(submap_j_texture.pixels.alpha.at(flat_index) == 255))) {
            ok = false;
          }
        }
      };
      ...
      if (ok) for(const auto& submap_j : submap_poses_) {
          if (id_i == submap_j.first) continue;
          validate_submap(submap_j.first);
          if(!ok) {
            submap_hint = cartographer::common::make_unique<cartographer::mapping::SubmapId>(submap_j.first);
            break;
          }
      }
      if (ok) {
...
std::pair<std::shared_ptr<cartographer::io::SubmapTexture>,
          std::shared_ptr<cartographer::io::SubmapTexture>> Detector::handleNewSubmapTexture(
    const cartographer::mapping::SubmapId &id,
    const cartographer::io::SubmapTexture &new_texture, const bool update) {

  auto texture_filtered_ptr = std::make_shared<cartographer::io::SubmapTexture>(new_texture);
  auto frontier_texture_ptr = std::make_shared<cartographer::io::SubmapTexture>(new_texture);
  ...
  std::vector<std::pair<Eigen::Array2i, std::unique_ptr<cartographer::mapping::SubmapId> > > submap_frontier_cells;
  for (int i = 0; i < texture_filtered.pixels.intensity.size(); i++) {
    ...
    if (is_unknown(i)) {
      ...
      if (free_neighbours >= 3 && unknown_neighbours >= 3) {
        frontier_texture.pixels.intensity.at(i) = 255; frontier_texture.pixels.alpha.at(i) = 255;
        submap_frontier_cells.push_back(std::make_pair(FromFlatIndex(i, frontier_texture), std::unique_ptr<cartographer::mapping::SubmapId>(nullptr)));
      }
    }
  }
  ...
  if (update)
    publishUpdatedFrontiers();
  return std::make_pair(std::move(texture_filtered_ptr), std::move(frontier_texture_ptr));
}
